...
/*   Updated: 2023/11/13 13:18:43 by lkilpela         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/**
	...
	* 1. The function takes three parameters: two void* pointers s1 and s2 to 
     the memory
 * blocks to compare,
	and a size_t n representing the number of bytes to compare.
 *
 * 2. The void* pointers s1 and s2 are cast to unsigned char* pointers,
	because we n
 ...
 *
 * 4. Inside the loop,
	it checks if the byte pointed to by ptr1 is not equal to the
 * byte pointed to by ptr2.
 *
 * 5. If they are not equal, it returns the difference between the two bytes.

	* This will be a positive number if the byte in s1 is greater than the byte 
     in s2,
 * and a negative number if it's less.
 *
 * 6. If the bytes are equal,
	it increments both ptr1 and ptr2 to move to the next byte
 * in each memory block, and decrements n.
 *
 * 7. If the loop completes without finding any differing bytes,
